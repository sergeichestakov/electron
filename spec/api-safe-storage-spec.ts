...
describe('safeStorage module', () => {
  ...
  describe('safeStorage persists encryption key across app relaunch', () => {
    it('can decrypt after closing and reopening app', async () => {
      ...
      const encryptAppProcess = cp.spawn(process.execPath, [encryptAppPath, '--password-store=basic']);
      let stdout: string = '';
      ...
      try {
        ...
        const relaunchedAppProcess = cp.spawn(process.execPath, [appPath, '--password-store=basic']);
